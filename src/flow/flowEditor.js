...
class FlowEditor extends zrender.Group {
  ...
  execute(name, data, nodes) {
    ...
    switch (name) {
      ...
      case "createGroup":
        this.stack.execute(
          new cmd.CreateGroup(data.group, data.topGroup, this)
        );
        break;
      case "cancelGroup":
      ...
      case "changeGroup":
        ...
        if (mark == "head") {
          ...
          this.stack.execute(
            new cmd.ChangeGroup(group, oldData, newData, mark)
          );
        } else if (mark == "group") {
  ...
  refreshHistory() {
    this.zr.trigger("undoredo", {
      undo: !!this.stack.canUndo(),
      redo: !!this.stack.canRedo(),
    });
  }
  ...
  getSelectItems() {
    return this.nodes
      ...
      .concat(this.edges.filter((item) => item.selected));
  }
  ...
  multipleSelectEdges(nodes) {
    ...
    activeItem.forEach((n) => {
      if (n.nodeType == "node") {
        ...
        es.forEach((e) => {
          if (
            activeItem.indexOf(e.fromNode) > -1 &&
            activeItem.indexOf(e.toNode) > -1
          ) {
